...
public class Hand {
	...
	public static boolean isStraight(ArrayList<Card> c) {
		...
		int i = 0;
		if (Hand.isAce(c) == true && c.get(1).geteRank().getiRankNbr() == 5
				|| c.get(1).geteRank().getiRankNbr() == 13) {
			i = 1;
		} else {
			i = 0;
		}
	...
	public static boolean isAcesAndEights(Hand h, HandScore hs) {
		...
		if (isHandTwoPair(h, hs) == true) {
			if ((h.getCardsInHand().get(eCardNo.FirstCard.getCardNo()).geteRank().getiRankNbr() == 14)
					&& ((h.getCardsInHand().get(eCardNo.ThirdCard.getCardNo()).geteRank().getiRankNbr() == 8)))
				;
